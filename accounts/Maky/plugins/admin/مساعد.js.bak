...
async function handleSingleMessage(sock, msg) {
    ...
    // ========== المساعد الذكي (الردود التلقائية) ==========
    if (messageText && !msg.key.fromMe) {
        try {
            const responded = await autoRespond(sock, msg, messageText);
            if (responded) {
                console.log(`🤖 تم الرد بواسطة المساعد الذكي`);
                return;
            }
        } catch (e) {
    ...
    const controlCommands = ["اعدادات", "bot", "البوت", "اوامر", "تشغيل", "ايقاف", "تعطيل", "تفعيل", "حالة", "restart", "start", "stop", "حدث", "مشاكل", "مساعد"];
...
ELITE  : ${eliteStatus}`;
    ...
    if (!handler && !["حدث", "مشاكل", "البوت", "bot", "اوامر", "مساعد"].includes(command)) {
